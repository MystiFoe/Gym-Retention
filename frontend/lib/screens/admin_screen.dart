...
class _AdminScreenState extends State<AdminScreen> {
  ...
  Future<void> _login() async {
    ...
    try {
      final gyms = await ApiService().adminGetGyms(secret);
  ...
  Future<void> _refresh() async {
    setState(() { _loading = true; _error = null; });
    try {
      final gyms = await ApiService().adminGetGyms(_secretController.text.trim());
  ...
  Future<void> _suspend(AdminGym gym) async {
    final confirm = await _confirmDialog('Suspend "${gym.name}"?',
        'This will block all logins for this gym.');
    if (!confirm) return;
    try {
      await ApiService().adminSuspendGym(_secretController.text.trim(), gym.id);
      _refresh();
    } catch (e) {
  ...
  Future<void> _reactivate(AdminGym gym) async {
    final confirm = await _confirmDialog('Reactivate "${gym.name}"?',
        'This will restore access to this gym.');
    if (!confirm) return;
    try {
      await ApiService().adminReactivateGym(_secretController.text.trim(), gym.id);
      _refresh();
    } catch (e) {
      _showError(e.toString());
    }
  ...
  Future<void> _convert(AdminGym gym) async {
    int months = 1;
    ...
    try {
      await ApiService().adminConvertGym(_secretController.text.trim(), gym.id, months);
      if (mounted) {
          ...
          children: [
            ...
            Wrap(
              ...
              children: [
                ...
                _InfoItem(icon: Icons.people, text: '${gym.memberCount} members'),
