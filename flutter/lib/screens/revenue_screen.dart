...
class RevenueScreen extends StatefulWidget {
  ...
  const RevenueScreen({super.key, this.refreshTrigger = 0});
...
class _RevenueScreenState extends State<RevenueScreen> {
  ...
  void didUpdateWidget(RevenueScreen oldWidget) {
    ...
    if (widget.refreshTrigger != oldWidget.refreshTrigger) {
      setState(() => revenueFuture = ApiService().getRevenue());
    }
  }
      ...
      appBar: AppBar(
        ...
        actions: [
          IconButton(
            ...
            onPressed: () async {
              await ApiService().logout(); // 🔥 VERY IMPORTANT
              final prefs = await SharedPreferences.getInstance();
              prefs.clear();
              authNotifier.update(false, role: '');
              if (!mounted) return;
              Future.microtask(() {
                AppRoutes.router.goNamed(RouteNames.login);
              });
            },
          ),
        ],
          ...
          if (snapshot.hasError) {
            ...
            if (err.contains('session') || err.contains('unauthorized') || err.contains('token') || err.contains('401')) {
              WidgetsBinding.instance.addPostFrameCallback((_) {
                authNotifier.update(false, role: '');
                AppRoutes.pushNamed(RouteNames.login);
              });
              return const Center(child: CircularProgressIndicator());
            }
            return Center(
                  ...
                  Row(
                    children: [
                      Expanded(
                        child: _KPICard(
                          title: 'Customers\nRecovered',
                          value: '${metrics?.totalRecoveredCustomers ?? 0}',
                          icon: Icons.people,
                          color: Colors.blue,
                        ),
                      ),
                    ...
                    Center(
                      child: Column(
                        children: [
                          ...
                          Text('Complete tasks with "Plan Renewed" outcome to track revenue',
                      ...
                      child: Column(
                        children: [
                          ...
                          Container(
                            ...
                            child: Row(
                              children: [
                                Expanded(flex: 3, child: Text('Customer', style: TextStyle(fontWeight: FontWeight.bold, color: Colors.grey[700], fontSize: 12))),
                                      ...
                                      child: Row(
                                        children: [
                                          CircleAvatar(
                                            ...
                                            child: Text(
                                              rec.customerName.isNotEmpty ? rec.customerName[0].toUpperCase() : '?',
                                              style: const TextStyle(color: Colors.green, fontSize: 12, fontWeight: FontWeight.bold),
                                            ),
                                          ),
                                          const SizedBox(width: 8),
                                          Flexible(
                                            child: Text(rec.customerName,
