...
describe('POST /api/tasks', () => {
    it('201: creates task with explicit trainer_id', async () => {
        q([{ is_blocked: false }], [{ id: MEMBER_ID }], // member existence check
    ...
    it('400: member has no trainer assigned and none provided', async () => {
        q([{ is_blocked: false }], [{ id: MEMBER_ID }], [{ assigned_trainer_id: null, name: 'Solo Member' }]);
        const res = await (0, supertest_1.default)(app)
            .post('/api/tasks')
            .set('Authorization', `Bearer ${OWNER_TOKEN}`)
            .send({ member_id: MEMBER_ID, task_type: 'call' });
    ...
    it('404: explicit trainer_id not found in gym', async () => {
        q([{ is_blocked: false }], [{ id: MEMBER_ID }], // member found
        []);
        const res = await (0, supertest_1.default)(app)
            .post('/api/tasks')
            .set('Authorization', `Bearer ${OWNER_TOKEN}`)
    ...
    it('401: unauthenticated returns 401', async () => {
        const res = await (0, supertest_1.default)(app).post('/api/tasks').send({ member_id: MEMBER_ID, task_type: 'call' });
...
describe('GET /api/tasks', () => {
    ...
    it('200: trainer gets tasks (authorize owner|trainer)', async () => {
        q([{ is_blocked: false }], [{ total: '1' }], [{ ...MOCK_TASK, member_name: 'Alice', trainer_name: 'Jane' }]);
        const res = await (0, supertest_1.default)(app)
            .get('/api/tasks')
...
describe('PATCH /api/tasks/:id', () => {
    it('200: trainer completes task with outcome=called', async () => {
        q([{ is_blocked: false }], [{ member_id: MEMBER_ID }], // UPDATE task RETURNING member_id
        [{ fcm_token: null }]);
        const res = await (0, supertest_1.default)(app)
    ...
    it('200: outcome=renewed creates revenue record', async () => {
        q([{ is_blocked: false }], [{ member_id: MEMBER_ID }], // UPDATE task
    ...
    it('200: all valid outcome values are accepted', async () => {
        ...
        for (const outcome of validOutcomes) {
            mockQuery.mockReset();
            mockQuery.mockResolvedValue({ rows: [], rowCount: 0 });
            q([{ is_blocked: false }], [{ member_id: MEMBER_ID }], []);
            const res = await (0, supertest_1.default)(app)
                .patch(`/api/tasks/${TASK_ID}`)
                .set('Authorization', `Bearer ${TRAINER_TOKEN}`)
                .send({ outcome });
    ...
    it('401: unauthenticated returns 401', async () => {
        const res = await (0, supertest_1.default)(app).patch(`/api/tasks/${TASK_ID}`).send({ outcome: 'called' });
        expect(res.status).toBe(401);
    });
