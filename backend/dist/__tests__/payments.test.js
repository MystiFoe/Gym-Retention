...
const TRAINER_TOKEN = jsonwebtoken_1.default.sign({ id: 'tr-uid', gym_id: GYM_ID, role: 'trainer' }, JWT_SECRET, { expiresIn: '1h' });
const MEMBER_TOKEN = jsonwebtoken_1.default.sign({ id: 'mem-uid', member_id: MEMBER_ID, gym_id: GYM_ID, role: 'member' }, JWT_SECRET, { expiresIn: '30d' });
...
describe('GET /api/payments', () => {
    it('200: owner gets paginated payment list', async () => {
        q([{ is_blocked: false }], [MOCK_PAYMENT_ROW], [{ total: '1', total_amount: '150000' }]);
    ...
    it('401: unauthenticated returns 401', async () => {
        const res = await (0, supertest_1.default)(app).get('/api/payments');
...
describe('GET /api/payments/report', () => {
    ...
    it('401: unauthenticated returns 401', async () => {
        const res = await (0, supertest_1.default)(app).get('/api/payments/report');
        expect(res.status).toBe(401);
    });
...
describe('POST /api/payments/create-order', () => {
    it('200: member creates a Razorpay order successfully', async () => {
        q([{ is_blocked: false }], [{ razorpay_key_id: 'rzp_test_key', razorpay_key_secret: 'rzp_test_secret' }]);
    ...
    it('400: gym has no Razorpay configured', async () => {
        q([{ is_blocked: false }], [{ razorpay_key_id: null, razorpay_key_secret: null }]);
        const res = await (0, supertest_1.default)(app)
            .post('/api/payments/create-order')
            .set('Authorization', `Bearer ${MEMBER_TOKEN}`)
            .send({ amount: 100 });
    ...
    it('401: unauthenticated returns 401', async () => {
        const res = await (0, supertest_1.default)(app).post('/api/payments/create-order').send({ amount: 100 });
        expect(res.status).toBe(401);
    });
...
describe('POST /api/payments/verify', () => {
    ...
    function makeValidSignature(secret, orderId, paymentId) {
    ...
    it('200: member verifies payment with valid signature', async () => {
        ...
        q([{ is_blocked: false }], [{ razorpay_key_secret: secret }], // gym secret lookup
        [{ id: PAYMENT_ID }]);
        const res = await (0, supertest_1.default)(app)
    ...
    it('400: invalid signature returns 400', async () => {
        q([{ is_blocked: false }], [{ razorpay_key_secret: 'correct_secret' }]);
        const res = await (0, supertest_1.default)(app)
            .post('/api/payments/verify')
            .set('Authorization', `Bearer ${MEMBER_TOKEN}`)
            .send({
    ...
    it('400: gym has no Razorpay secret configured', async () => {
        q([{ is_blocked: false }], [{ razorpay_key_secret: null }]);
        const res = await (0, supertest_1.default)(app)
            .post('/api/payments/verify')
            .set('Authorization', `Bearer ${MEMBER_TOKEN}`)
            .send({
    ...
    it('404: payment record not found after signature check', async () => {
        const secret = 'gym_test_secret';
        const sig = makeValidSignature(secret, ORDER_ID, PAYMENT_REF);
        q([{ is_blocked: false }], [{ razorpay_key_secret: secret }], []);
        const res = await (0, supertest_1.default)(app)
            .post('/api/payments/verify')
            .set('Authorization', `Bearer ${MEMBER_TOKEN}`)
            .send({
    ...
    it('401: unauthenticated returns 401', async () => {
        const res = await (0, supertest_1.default)(app).post('/api/payments/verify').send({});
        expect(res.status).toBe(401);
    });
