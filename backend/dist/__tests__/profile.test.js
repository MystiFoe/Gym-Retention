...
describe('GET /api/profile', () => {
    it('200: returns owner profile with gym info', async () => {
        q([{ is_blocked: false }], [MOCK_OWNER_PROFILE]);
    ...
    it('200: returns trainer profile', async () => {
        q([{ is_blocked: false }], [MOCK_TRAINER_PROFILE]);
        const res = await (0, supertest_1.default)(app)
            .get('/api/profile')
    ...
    it('401: unauthenticated returns 401', async () => {
        const res = await (0, supertest_1.default)(app).get('/api/profile');
...
describe('PUT /api/profile', () => {
    ...
    it('409: email already in use by another account', async () => {
        q([{ is_blocked: false }], [], // UPDATE gyms SET owner_name (always runs first)
        [{ id: 'other-user-id' }]);
        const res = await (0, supertest_1.default)(app)
            .put('/api/profile')
            .set('Authorization', `Bearer ${OWNER_TOKEN}`)
