...
describe('GET /api/dashboard/kpis', () => {
    it('200: returns KPI counts and revenue for owner', async () => {
        q([{ is_blocked: false }], [{ active_members: '10', at_risk_members: '3', high_risk_members: '2', total_members: '15' }], [{ total_revenue: '50000' }]);
    ...
    it('200: zero members and zero revenue', async () => {
        q([{ is_blocked: false }], [{ active_members: '0', at_risk_members: '0', high_risk_members: '0', total_members: '0' }], [{ total_revenue: '0' }]);
        const res = await (0, supertest_1.default)(app)
            .get('/api/dashboard/kpis')
            .set('Authorization', `Bearer ${OWNER_TOKEN}`);
        expect(res.status).toBe(200);
    ...
    it('200: fractional revenue is parsed correctly', async () => {
        q([{ is_blocked: false }], [{ active_members: '5', at_risk_members: '1', high_risk_members: '0', total_members: '6' }], [{ total_revenue: '12345.67' }]);
        const res = await (0, supertest_1.default)(app)
            .get('/api/dashboard/kpis')
            .set('Authorization', `Bearer ${OWNER_TOKEN}`);
        expect(res.status).toBe(200);
    ...
    it('401: unauthenticated request returns 401', async () => {
        const res = await (0, supertest_1.default)(app).get('/api/dashboard/kpis');
...
describe('GET /api/revenue', () => {
    ...
    it('200: monthly breakdown maps total and count correctly', async () => {
        q([{ is_blocked: false }], [{ total_recovered_members: '1', total_revenue: '1500', revenue_this_month: '1500', revenue_this_year: '1500' }], [{ id: 'r1', member_id: 'm1', member_name: 'Alice', task_id: 't1', action: 'renewal', revenue_recovered: 1500, tracked_at: new Date().toISOString() }], [{ month: '2026-05-01', total_revenue: '1500', recovery_count: '1' }]);
        const res = await (0, supertest_1.default)(app)
            .get('/api/revenue')
            .set('Authorization', `Bearer ${OWNER_TOKEN}`);
        expect(res.status).toBe(200);
    ...
    it('401: unauthenticated request returns 401', async () => {
        const res = await (0, supertest_1.default)(app).get('/api/revenue');
        expect(res.status).toBe(401);
    });
