...
const MEMBER_TOKEN  = jwt.sign({ id: USER_ID,        gym_id: GYM_ID, role: 'member'  }, JWT_SECRET, { expiresIn: '1h' });
...
describe('GET /api/members', () => {
  ...
  it('200: owner gets paginated member list', async () => {
    q(
      [{ is_blocked: false }],
      [{ total: '2' }],
      [MOCK_MEMBER_ROW, { ...MOCK_MEMBER_ROW, id: 'mem-2', name: 'Member Two' }],
    );
  ...
  it('200: trainer sees only their assigned members', async () => {
    q(
      [{ is_blocked: false }],
      [{ id: 'trainer-row-id' }],   // trainer lookup
      [{ total: '1' }],
      [MOCK_MEMBER_ROW],
    );
...
describe('POST /api/members', () => {
  ...
  it('201: creates member successfully', async () => {
    q(
      [{ is_blocked: false }],
      [],                         // phone uniqueness check → not taken
  ...
  it('409: duplicate phone returns 409', async () => {
    q(
      [{ is_blocked: false }],
      [{ id: 'existing', is_deleted: false }],  // phone taken
    );
  ...
  it('401: unauthenticated', async () => {
    const res = await request(app).post('/api/members').send(VALID_BODY);
    expect(res.status).toBe(401);
  });
});
...
describe('PUT /api/members/:id', () => {
  ...
  it('200: owner updates member', async () => {
    q(
      [{ is_blocked: false }],
      [],                           // phone uniqueness → OK
      [{ id: MEMBER_ID, updated_at: new Date() }], // UPDATE result
    );
...
describe('DELETE /api/members/:id', () => {
  ...
  it('200: owner soft-deletes member', async () => {
    q(
      [{ is_blocked: false }],
      [{ id: MEMBER_ID }],  // UPDATE returns row
    );
...
describe('DELETE /api/members/:id/data (GDPR)', () => {
  ...
  it('200: erases PII and returns success', async () => {
    q(
      [{ is_blocked: false }],
      [{ id: MEMBER_ID }],    // member exists check
    );
