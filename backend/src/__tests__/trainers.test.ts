...
describe('POST /api/trainers', () => {
  ...
  it('201: owner creates trainer successfully', async () => {
    q(
      [{ is_blocked: false }],  // authenticate gym block
      ...
      [MOCK_TRAINER_ROW],        // INSERT trainers RETURNING
  ...
  it('409: duplicate email returns 409', async () => {
    q(
      [{ is_blocked: false }],
      [{ id: 'existing-user' }], // email already registered
    );
  ...
  it('401: unauthenticated returns 401', async () => {
    const res = await request(app).post('/api/trainers').send(VALID_TRAINER_BODY);
...
describe('GET /api/trainers/me', () => {
  ...
  it('401: unauthenticated returns 401', async () => {
    const res = await request(app).get('/api/trainers/me');
    expect(res.status).toBe(401);
  });
});
...
describe('PATCH /api/trainers/:id', () => {
  ...
  it('404: trainer not found in gym returns 404', async () => {
    q(
      [{ is_blocked: false }],
      // BEGIN default
      [],  // UPDATE trainers returns no rows → 404
      // ROLLBACK default
    );
  ...
  it('409: new email already in use', async () => {
    q(
      ...
      [{ id: 'other-user-id' }],   // email taken by another user
      // ROLLBACK uses base default
    );
  ...
  it('401: unauthenticated returns 401', async () => {
    const res = await request(app).patch(`/api/trainers/${TRAINER_ID}`).send({ name: 'X', phone: '1234567890' });
    expect(res.status).toBe(401);
  });
});
...
describe('DELETE /api/trainers/:id', () => {
  ...
  it('200: owner soft-deletes trainer and marks user deleted', async () => {
    q(
      [{ is_blocked: false }],
      [{ user_id: USER_ID }],  // UPDATE trainers RETURNING user_id
  ...
  it('200: trainer not in gym still returns 200 (idempotent delete)', async () => {
    q(
      [{ is_blocked: false }],
      // UPDATE trainers → no rows returned (no user_id to clean up)
    );
  ...
  it('401: unauthenticated returns 401', async () => {
    const res = await request(app).delete(`/api/trainers/${TRAINER_ID}`);
    expect(res.status).toBe(401);
  });
...
describe('POST /api/trainers/:id/assign-members', () => {
  ...
  it('200: assigns members to trainer and returns real count', async () => {
    q(
      ...
      [{ count: '2' }],     // COUNT assigned members
      // UPDATE trainer count (base default)
      // COMMIT uses base default
    );
...
describe('POST /api/trainers/bulk-import', () => {
  ...
  it('401: unauthenticated returns 401', async () => {
    const res = await request(app).post('/api/trainers/bulk-import').send({ trainers: [] });
    expect(res.status).toBe(401);
  });
