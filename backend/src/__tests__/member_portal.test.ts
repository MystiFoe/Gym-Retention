...
const MEMBER_TOKEN = jwt.sign(
  { id: USER_ID, member_id: MEMBER_ID, gym_id: GYM_ID, role: 'member' },
  JWT_SECRET, { expiresIn: '30d' }
);
const MEMBER_TOKEN_NO_ID = jwt.sign(
  { id: USER_ID, gym_id: GYM_ID, role: 'member' },
  JWT_SECRET, { expiresIn: '30d' }
);
const OWNER_TOKEN = jwt.sign(
  { id: 'owner-uid', gym_id: GYM_ID, role: 'owner' },
...
describe('GET /api/customer/profile', () => {
  ...
  it('200: member_id in JWT, profile found', async () => {
    q(
      [{ is_blocked: false }],  // authenticate gym check
  ...
  it('404: member_id not in JWT + resolveMemberId finds nothing → 404', async () => {
    q(
      [{ is_blocked: false }],
      [],  // strategy 1 (user_id) → empty
      [],  // strategy 2 (email/phone) → empty
    );
...
describe('GET /api/customer/attendance', () => {
  ...
  it('200: returns attendance array', async () => {
    q(
      [{ is_blocked: false }],
      [{ date: '2026-05-01', status: 'present', source: 'mobile' }],
    );
  ...
  it('401: unauthenticated request rejected', async () => {
    const res = await request(app).get('/api/customer/attendance');
    expect(res.status).toBe(401);
  });
});
...
describe('GET /api/customer/payments', () => {
  ...
  it('200: returns payment history', async () => {
    q(
      [{ is_blocked: false }],
      [{ id: 'pay-1', amount: 150000, currency: 'INR', status: 'completed', payment_method: 'razorpay', description: null, created_at: '2026-01-01' }],
      [{ count: '1' }],
    );
...
describe('POST /api/attendance/checkin', () => {
  ...
  it('200: already_marked=true if attendance exists today', async () => {
    q(
      [{ is_blocked: false }],
      [{ id: 'att-1', source: 'mobile' }],  // already marked
    );
