...
describe('POST /api/invites', () => {
  ...
  it('201: staff invite code created', async () => {
    q(
      [{ is_blocked: false }],
      [{ last_value: '0' }],  // id_sequences upsert (INSERT ... ON CONFLICT)
      [{ last_value: '1' }],  // id_sequences UPDATE
      [{ id: TRAINER_ID }],   // INSERT trainers pending slot
    );
...
describe('GET /api/invites/:code', () => {
  ...
  it('200: valid code returns invite info', async () => {
    ...
    const res = await request(app).get('/api/invites/ABCD1234');
  ...
  it('404: invalid or expired code', async () => {
    ...
    const res = await request(app).get('/api/invites/BADCODE1');
