...
describe('GET /api/biometric/devices', () => {
  ...
  it('200: returns list of registered devices', async () => {
    q(
      [{ is_blocked: false }],
      [MOCK_DEVICE, { ...MOCK_DEVICE, id: 'dev-2', serial_number: 'ZK002' }],
    );
...
describe('DELETE /api/biometric/devices/:id', () => {
  ...
  it('401: unauthenticated returns 401', async () => {
    const res = await request(app).delete(`/api/biometric/devices/${DEVICE_ID}`);
    expect(res.status).toBe(401);
  });
...
describe('GET /api/biometric/mappings', () => {
  ...
  it('200: filters by serial number query param', async () => {
    q(
      [{ is_blocked: false }],
      [{ id: 'map-1', serial_number: 'ZK001', device_user_id: '1', member_id: MEMBER_ID }],
    );
  ...
  it('401: unauthenticated returns 401', async () => {
    const res = await request(app).get('/api/biometric/mappings');
    expect(res.status).toBe(401);
  });
...
describe('PUT /api/biometric/mappings', () => {
  ...
  it('200: maps a device user to a member', async () => {
    q(
      [{ is_blocked: false }],
      [{ id: 'map-1', gym_id: GYM_ID, serial_number: 'ZK001', device_user_id: '5', member_id: MEMBER_ID }],
    );
  ...
  it('200: clears a mapping by setting member_id to null', async () => {
    q(
      [{ is_blocked: false }],
      [{ id: 'map-1', gym_id: GYM_ID, serial_number: 'ZK001', device_user_id: '5', member_id: null }],
    );
...
describe('POST /api/attendance/qr', () => {
  ...
  it('200: marks attendance for member via QR scan', async () => {
    q(
      [{ is_blocked: false }],
      [{ id: MEMBER_ID, name: 'Alice' }],  // member check
      // INSERT attendance (default)
      // UPDATE last_visit_date (default)
    );
