...
describe('POST /api/auth/forgot-password', () => {
  ...
  it('200: sends OTP email when user exists (email enumeration safe)', async () => {
    q(
      [{ id: USER_ID }],  // user found
      // DELETE existing tokens (default)
      // INSERT new token (default)
    );
...
describe('POST /api/auth/verify-reset-otp', () => {
  ...
  it('200: verifies correct OTP and returns reset_token', async () => {
    q(
      [{ id: USER_ID }],
      [{ id: TOKEN_ID, token: 'otp:123456', expires_at: FUTURE }],
      // UPDATE token (default)
    );
  ...
  it('400: wrong OTP code returns 400', async () => {
    q(
      [{ id: USER_ID }],
      [{ id: TOKEN_ID, token: 'otp:999999', expires_at: FUTURE }],
    );
  ...
  it('400: expired OTP returns 400', async () => {
    ...
    q(
      [{ id: USER_ID }],
      [{ id: TOKEN_ID, token: 'otp:123456', expires_at: PAST }],
    );
...
describe('POST /api/auth/reset-password', () => {
  ...
  it('200: resets password with valid token', async () => {
    q(
      [{ id: TOKEN_ID, user_id: USER_ID, expires_at: FUTURE, used_at: null }],
      // UPDATE users password_hash (default)
      // UPDATE tokens used_at (default)
    );
...
describe('POST /api/auth/send-otp', () => {
  ...
  it('200: sends OTP when user exists', async () => {
    q(
      [{ id: USER_ID }],  // user found
      // DELETE existing otp (default)
      // INSERT new otp (default)
    );
...
describe('POST /api/auth/verify-otp', () => {
  ...
  it('200: verifies valid OTP and marks email as verified', async () => {
    q(
      [{ id: 'otp-id', expires_at: FUTURE, used_at: null }],
      // UPDATE otp_codes used_at (default)
      // UPDATE users email_verified (default)
    );
