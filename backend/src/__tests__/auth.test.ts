...
describe('POST /api/auth/login', () => {
  ...
  it('200: owner login returns accessToken + refreshToken', async () => {
    q(
      [{ id: USER_ID, gym_id: GYM_ID, phone_or_email: 'o@g.com', password_hash: '$h', role: 'owner' }],
  ...
  it('200: member login includes member_id in response', async () => {
    q(
      [{ id: USER_ID, gym_id: GYM_ID, phone_or_email: 'm@g.com', password_hash: '$h', role: 'member' }],
      [{ subscription_status: 'trial', is_blocked: false }],
      [{ id: MEMBER_ID }],  // strategy 1: user_id match
    );
    mockBcryptCompare.mockResolvedValueOnce(true);
  ...
  it('200: member login works even when member_id not found (member_id absent from token)', async () => {
    q(
      [{ id: USER_ID, gym_id: GYM_ID, phone_or_email: 'm2@g.com', password_hash: '$h', role: 'member' }],
      [{ subscription_status: 'trial', is_blocked: false }],
      [],  // strategy 1: no user_id match
      [],  // strategy 2: no email/phone match
    );
    mockBcryptCompare.mockResolvedValueOnce(true);
...
describe('POST /api/auth/refresh', () => {
  ...
  it('200: valid token issues new pair', async () => {
    const rt = jwt.sign({ id: USER_ID, gym_id: GYM_ID }, REFRESH_SECRET, { expiresIn: '7d' });
